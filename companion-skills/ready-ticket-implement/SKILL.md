...
description: "Implement one existing IIS Ready Ticket with an explicitly configured optional roster of 0 to 3 concurrent read-only implementation auditors. Use for exact Ready Ticket implementation and implementer self-check. This skill does not perform, replace, reinterpret, or adjudicate the separate verification authority."
---
...
- Count `1~3`의 각 활성 slot에는 exact `Model`과 exact `Reasoning Depth`가 모두 필요하다. 누락되면 `AUDIT CONFIGURATION REQUIRED: BINDING`이다.
- 첫 source change 전에 host가 caller/user가 지정한 exact auditor binding으로 해당 slot을 실제 시작할 수 있어야 한다. 요청한 exact binding의 start/spawn 자체가 실패하면 해당 slot을 `AUDITOR BLOCKED: EXACT BINDING UNAVAILABLE`로 보고한다. Exact requested slot이 성공적으로 시작된 뒤에는 auditor의 self-reported model name, reasoning depth, runtime metadata 또는 display label은 diagnostic only이며 slot validity, re-audit 필요 여부, `IMPLEMENT` completion을 바꾸지 않는다.
