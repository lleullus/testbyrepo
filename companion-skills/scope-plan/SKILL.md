...
- **Repository investigation:** exact immutable artifact and recorded identity, or `None supplied`. Preserve any load-bearing evidence anchors and their current-applicability conditions that the method will use; a summary is not a replacement.
...
- **Known correction evidence:** exact predecessor Verify/Probe finding and primary evidence, repeatable reproducer or exact limitation, current target and settled effects, or `None`.
...
- Optional exact predecessor Verify/Probe finding, reproducible evidence and current target for corrective planning.
- Exact outside-Project-Root `plan_review_path` selected for this invocation; never discover a latest review. Select a location that remains readable by the admitted implementation and any current-request correction handoff; a worker-local temporary path with no successor access is a known retention limit.
...
Failure frontier and executable discriminators: <exact Plan anchors and project check/environment disposition>
Evidence and handoff accessibility: <actual independent invocation, primary evidence references and successor-readability limits>
Limitations: None | <exact unavailable evidence/independence/capability/currentness>
Completion: COMPLETE | BLOCKED | PARTIAL
```

`COMPLETE` requires a current independent review with `ADMIT` for this exact Scope and an accessible handoff for dependent implementation. A missing reviewer, `REVISE`, `EVIDENCE_NEEDED`, stale or inaccessible load-bearing artifact, or smaller result is `PARTIAL` or `BLOCKED` according to the actual boundary. Preserve the exact next owner/action. This role never marks Scope `done`, implements it, verifies it, or starts a later Scope.
