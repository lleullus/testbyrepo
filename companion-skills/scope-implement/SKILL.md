...
A correction faithfully realizing the same bound decisions may change private helpers, internal names or equivalent local structure. Return to Product-Thesis when success/preservation/identity would need to be reselected or a Thesis-bound construction decision/factual premise cannot hold in the actual code; return to Transition when order/intermediate authority/invariant is wrong; return to Planner when only a Plan method, actual state/effect owner assumption, interface/readback method or safety premise is refuted. Fallback, retry, cleanup or recovery work must not silently decide a new user result. Small diff, private code or technical vocabulary does not make a decision implementation-local. If the choice determines what may be lost, what counts as current, when an effect is complete, or which result is authoritative, return it to the bound decision owner. Fresh invocation preserves selected actor/model/effort and requires previous effects to settle; do not keep a completed invocation open or substitute Main silently.
