...
2. Run acceptance tests only through `ready_argv acceptance`, with `provenance_kind` exactly `LOCAL_PATH`, `LOCAL_SQLITE`, or `EXTERNAL_HTTP_PROVIDER`. The runtime uses Python/JS/TS structural local import closure and JS/TS global/member/destructuring/alias-aware mocking/interception checks; basename or text mention alone is not provenance. Resolved-argv selected test roots and `evidence_paths` must be the exact same canonical set, and ambiguous/unsupported selectors fail closed. Package runners fingerprint original/resolved argv and `package.json` and reject configured exact adjacent `pre<name>`/`post<name>` lifecycle hooks before execution. It rejects unverifiable custom runner/MCP execution. When the authored authority is direct source/artifact/canonical inspection rather than a test, use the native read path and let `ready_verify_guard admit` bind that current successful read as direct-inspection provenance; do not invent a test merely to satisfy the runtime.
3. For clean local PASS evidence, fingerprint original/resolved command argv, production/evidence import closures, actual dependency/config, authoritative readback, runner config and current mutation revision with `mock_taint: false`.
4. The current runtime does not support execution/readback correlation for `EXTERNAL_HTTP_PROVIDER`; it runs neither command nor readback argv and records acceptance as `INCONCLUSIVE`. The affected flow/AC remains `INCONCLUSIVE`, and this provenance cannot support `VERIFIED`.
5. Before candidate `VERIFIED`, `ready_verify_guard admit` recomputes the same exact selected-test-root/`evidence_paths` binding and full current fingerprint and rejects mock-tainted, stale, incomplete, non-passing, or unavailable-readback acceptance provenance. Current clean direct-inspection provenance remains admissible for an authored direct canonical inspection boundary. The runtime does not issue the AC or Whole-Ticket verdict itself.
