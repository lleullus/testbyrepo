...
Dispatch neutrally: do not prescribe COMPLETE/None, promote the verifier's success into an expected Coverage conclusion, or substitute a summary for primary evidence. Copy quoted historical outcomes only as labeled history. Coverage must read the exact evidence and may return findings or limits. Use the pinned release's explicit readable role paths; unavailable loaded identity stays unknown, not inferred from a current pointer.
