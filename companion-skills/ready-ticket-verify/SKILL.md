...
description: "Verify one existing IIS Ready Ticket against a stable current implementation target. Use when Codex must produce and report an integrated runtime verification scenario, execute or inspect every authored Verification flow according to its approved disposition with one Main verifier, optionally attach 0 to the current top-level AC count concurrent read-only AC Runtime Auditors to the same execution, adjudicate every AC, and own the final Ticket verification verdict and terminal done transition."
---
