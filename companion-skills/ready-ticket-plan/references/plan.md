# Planner: execution method, not product authority

## Unit and evidence

Read each exact Ticket in full, its current Parent Spec and applicable Behavior/UI authority. Fix the approved observable obligation and preserved paths before choosing a method. A method may cover one Ticket or the actual impact span of an important shared decision. Review producer/consumer requirements and costly commitments together before the first dependent implementation. Do not require every unrelated Ticket's detailed plan first.

Check actual ownership of each applicable parent obligation, including within a shared outcome ordinal. An exact sibling may own it and approved future work may be excluded; an ownerless due-now obligation returns to To Tickets (or To Spec/the meaning owner if the parent is wrong). Its absence from this AC does not authorize a narrower method or silent authority edits.

Use `<Project-Root>/docs/planning/work/<work-slug>/plans/PLAN-NNN.md`; the number is a locator, not a durable obligation ID. Reference an exact shared file/section when a shared section suffices. Keep references acyclic; consolidate a cyclic shared decision rather than building a graph engine. Include every used common plan in the eventual review's `plans` identities.
...
6. Actual implementation self-check and final acceptance/readback path. For each relevant failure, connect the cheapest valid discriminating check, the confirmed same-assumption impact span, the minimum check through the real acceptance path, and the current-target condition for verifier handoff. Order checks by discriminating power, cost, authority, side effects, dependencies and initial state rather than a fixed test-category sequence. Where an environment is required, identify the artifact/build, service-start and current-instance readiness owner, scenario state changes, settlement/cleanup owner and known limits.
   Separate supplemental source/helper/double checks from proof through the required acceptance boundary. For a runtime outcome the latter must preserve its real entry, transition/effect and readback; an environment limit is a missing observation, not permission to redefine success. State necessary initial conditions: post-cleanup absence alone does not prove deletion of a previously existing artifact. Timing evidence must measure the authored milestone, not substitute onset for settlement or alter the threshold.
...
For each unresolved premise that can be safely decided during implementation, state together:

- `plan_anchor`: exact current section;
- `permitted_initial_work`: bounded first work;
- `discriminating_observation`: actual readback separating support/refutation;
...
5. **R5 — Refresh affected review.** Reconfirm directly implicated contract/source/schema/search/runtime premises, check repair-induced state/order/ownership paths, and pass the affected scope plus exact evidence limits to a current independent reviewer. Material method or premise changes require a new affected judgment; non-material carry-forward is valid only when the reviewer directly checks the delta and premise non-impact against the current Plan. No separate exploration invocation or carry-forward identity is created.
