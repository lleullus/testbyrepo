...
Audit both directions: Ticket clauses must trace to approved parent meaning, and each obligation applicable to this Increment must have actual current Ticket/AC/Flow ownership. One covered parent ordinal does not establish every obligation inside it. Read the exact sibling when ownership is elsewhere; preserve approved future/Non-Goal exclusions. An ownerless parent requirement cannot be dismissed because the authored AC omitted it. Correct faithful decomposition locally; a downstream role discovering that gap returns it to To Tickets, or to To Spec/the meaning owner when the parent itself is wrong. No downstream role silently repairs or weakens authority to proceed.
