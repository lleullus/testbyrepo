...
Read the actual Thesis source reached through the exact immutable Scope binding, or the saved direct Thesis source, before forming the provisional frame and final shared understanding. Apply its concrete behaviors, causal links and success distinctions to the admitted Increment without importing future scope. Preserve the exact source binding for To Spec: new direct work uses `iis-product-meaning/v2`; faithful legacy Scope continuation retains its existing binding. Detailed states, transitions, recovery, ordering, concurrency and UI remain Behavior/UI responsibilities.
