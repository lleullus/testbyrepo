...
Adaptive execution settings do not redefine common planning ownership. Shared Product Thesis storage/source binding applies to ordinary and Adaptive planning alike; it is not Adaptive-only metadata. Preserve the owning Scope/Matt/Behavior/UI contracts and change their common source-consumption rules together when that shared contract changes.
...
1. **Run Contract closure and terminal discipline** — before the first downstream planning mutation, normalize current authority into one compact invocation-local contract under [Goal and required-item coverage](09-run-contract.md#goal-and-required-item-coverage-invariant). Preserve the assigned Goal, distinguish required/candidate items, fix Required Item Policy, record Implementation/Verification independently, preserve or user-confirm enabled SUBAGENT model/effort, and close a sufficient Boundary/Predicate/Readback and Run Contract Approval Gate. Ask only for a material unresolved field; recommendations are not model consent. The gate defaults to `not_required`; only an affirmative `/승인게이트` on the current explicitly active invocation sets `required`, renders the exact `CLOSED` contract, and STOPs before downstream planning/delivery mutation until direct user approval. It neither activates Adaptive nor adds downstream approval gates. Baseline owner STOPs keep their meaning; whole-run success additionally requires faithful Goal/required-item coverage and actual completion evidence.
...
6. **Planning re-entry/reshaping** — fresh evidence may send the current unit back to an earlier existing IIS planning leaf without a new user prompt when the Mandate authorizes it. The assigned Goal and Required Named Items remain outer obligations under `09-run-contract.md`; changing current construction scope or candidate means cannot weaken the whole-run promise.
...
A broader `NAMED_REQUIRED_ITEMS_DELIVERED`, `BOUNDED_OUTCOME_SATISFIED`, or `MANDATE_OUTCOME_SATISFIED` boundary does not turn one planning cycle into multi-Increment scope. After current delivery, Outer Main revalidates Goal/required-item coverage and uses fresh actual evidence; when the assigned result remains unmet and authorized construction is needed, it continues to Scope Shaper for exactly one new current Increment. A weak Predicate cannot turn unmet Goal into success.
...
- execution-plan preparation, implementation, and integrated verification are outside IIS Planning;
