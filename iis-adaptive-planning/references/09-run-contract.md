...
**Sniper Principle at closure:** for product or feature construction, derive the Goal Outcome from the durable utility the user is trying to obtain, not by restating a list of requested features, UI elements, or technical mechanisms. Preserve an exact named capability as Required when current authority makes that result mandatory; otherwise do not promote a proposed means into the purpose of the run. Do not invent competitive differentiation, market strategy, or broader product work that the user did not assign.
...
For product or feature construction, apply the same question specifically against **feature-list substitution**: could every named feature be present and the Completion Predicate pass while the durable utility for which the user requested the product is still absent? If yes, the form is not `CLOSED`. Recover the utility from clear Source Authority and correct the derived contract; ask the user only when that product meaning is genuinely unresolved. This check narrows success to the real assigned purpose and must not expand the invocation into unassigned strategy, polish, or adjacent features.
