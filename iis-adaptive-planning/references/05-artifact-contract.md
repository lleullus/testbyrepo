...
Execution plans are derived method artifacts, normally under the work slug's `plans/PLAN-NNN.md`, not canonical product authority or Adaptive state. Current `iis-plan-review/v1` is one exact outside-Project-Root handoff produced by the independent reviewer, with semantic rationale and current plan/common-plan/Ticket/authority identities. Carry the actual result/path unchanged as `plan_review_path` for the required Ticket; do not synthesize approval, search for latest reviews, copy the artifact into planning state, or create a plan progress ledger. Model selections remain invocation-local and the complete requested current ADMIT denominator alone closes READY_EXECUTION_PLANS. Final verification may consume exact plan/review navigation but does not require it; plan-only context drift is distinct from actual product-target drift. No separate post-implementation discovery artifact or binding is required.
