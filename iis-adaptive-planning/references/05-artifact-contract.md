...
A `READY TICKET HEURISTIC PROBE RESULT` and its current `Probe Machine Binding` are invocation-local delivery handoffs, not canonical IIS artifacts or required durable Adaptive companion artifacts. The machine binding remains the exact session-local runtime artifact produced outside Project Root by the owning Probe; Adaptive must preserve its exact path only for the current downstream verifier and must not copy, rewrite, normalize, or promote it into project planning state. Do not create project-local `HEURISTIC-PROBE-RESULT.md`, a copied machine binding, probe ledger, worker roster, evidence cache, probe state database, or retained workflow record merely to carry the gate. Preserve only the current result plus its required current binding in the active delivery context. If a material probe finding is later adjudicated by the verifier and changes the Adaptive corrective route, the existing verification-triage trace may cite that evidence; routine finding-free probes, lane inventories, worker/model rosters, machine-binding contents and probe progress are not Adaptive provenance events.

Implementation/verification checkpoint reports and Parent continuation decisions are also invocation-local delivery messages. They are not canonical IIS artifacts, Adaptive companion artifacts, trace events by default, progress heartbeats, checkpoint IDs/ledgers, or workflow state. Preserve them only in the active owner/Parent continuation context. If a checkpoint exposes a material authority fact that later changes terminal routing, record the resulting route only after the exact terminal owner result exists; do not persist the checkpoint itself as new provenance machinery.
