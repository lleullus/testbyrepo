...
The **Outer Main** owns invocation-local Run Contract closure/carry-forward, the owner-return event loop, fresh completion assessment, and the final caller result. It compares applicable obligations, existing acceptance owners, and current evidence/limits without issuing a second AC/Ticket verdict. IIS leaves own product planning; `ready-ticket-plan` owns execution-method preparation and independent start review; `ready-ticket-implement` owns implementation/self-check; exactly one `ready-ticket-verify` SUBAGENT owns discriminating scenarios and exact-Ticket semantic adjudication. After VERIFIED from ready, Outer Main obtains one independent read-only `ready-ticket-coverage` result before submitting the associated original opaque handle to `ready_finalize`. Only COMPLETE with no unresolved material finding or evidence gap permits success submission; other verdicts retain non-progressing finalization without Coverage. Outer Main is the sole Coverage/finalization caller, not a second verify wrapper.
...
For every successfully dispatched background implementation or verification owner, Outer Main follows the canonical [owner-return event loop](references/08-delivery-continuation.md#invocation-local-owner-return-event-loop): no progress/completion polling, status DM, or duplicate repository/runtime inspection during normal execution. It stands by once and resumes routing only when the host delivers the exact terminal owner result. A bounded snapshot is allowed only for an explicit user status or cancellation request, host-reported timeout/failure, malformed or missing terminal delivery, or real replacement/settlement diagnosis; a normally running owner returns immediately to passive fan-in without periodic monitoring.
...
Implementation and Verification are independent invocation fields. Preserve explicit `do not implement` and `do not verify` overrides. `Verification: no` never permits a verifier or Coverage call, a `done` claim, or a delivered/outcome boundary that still requires verification.
...
Adaptive triage consumes the exact readable `ready-ticket-verify` semantic result plus the exact host-delivered terminal handle, the exact Coverage terminal when performed, the actual caller `ready_finalize` result when submitted, and current evidence. A Coverage finding or PARTIAL/BLOCKED result may withhold success without changing VERIFIED; preserve the exact gap and route it under the existing owner rules, never fabricate a finalizer failure. The opaque handle is submitted only to `ready_finalize` and never copied into triage artifacts; readable binding identity and semantic verdict remain evidence only. Scenario reports, material observations, and other in-cycle evidence are verifier-owned navigation, not automatic defects. A preparation REVISE/EVIDENCE_NEEDED or PLAN_REVIEW_REQUIRED result is planning-method evidence, not verification.
...
Outer Main invokes `ready-ticket-plan` for requested preparation or before implementation lacking current ADMIT, forwards the actual outside-root `plan_review_path` to the actual implementing actor, and invokes implement only when Implementation is yes. That actor must pass stateless start/end admission. Verification yes dispatches exactly one `ready-ticket-verify` SUBAGENT for a stable current ready target; on its exact host terminal return, Outer Main follows the post-success Coverage sequence in `08-delivery-continuation.md` before eligible `ready_finalize` submission with only the associated host-delivered `terminal_handle`. The verifier never dispatches Coverage, and Outer Main does not duplicate it. Reuse applicable current model selections under `09-run-contract.md`; no new delivery switch or fixed model roster is added. Verification no leaves `ready` unchanged. After enabled owner work, re-evaluate the actual current Increment and Completion Predicate instead of assuming stage completion equals Run completion.
