...
Read the latest user assignment and exact applicable Thesis originals, including any adopted construction decision that constrains the current result. Inspect actual current product state, reusing supplied current investigation where sufficient. Read any explicitly applicable approved Transition Baseline and the current measured Block. No source is created merely to fill an input field.
...
Product-Thesis exploration or a supplied Completion Brief may identify a gap, but neither is a required handoff artifact or a readiness verdict. Consume exact adopted sources and deciding evidence at their actual strength. Preserve any Thesis-bound construction decision that constrains this Outcome; Plan may not replace it for convenience. Keep evidence-only unknowns separate from unresolved product choices/premises. A missing fact or unresolved Thesis-bound decision that can change the promised boundary or current Outcome prevents ready. A method-only choice does not block ready when all alternatives remain faithful, but do not classify an unresolved choice as method-only merely because it is expressed technically. After Thesis decisions change, reassess affected Outcome/Acceptance through existing re-entry rules. Do not silently bind an unregistered latest Thesis, bypass lifecycle closure, import every explored sibling or rewrite completed history.

## Transition contract

Use a Transition Baseline only when there is real migration/coexistence/cutover or intermediate authority/order/preservation geography to govern; technical complexity alone does not create one. When an approved BASELINE-NNN applies, reconcile the current measured Block and actual state against its entry/exit/readback, invariants, continuation/abort limits and remaining Goal obligations. A Block number does not establish runtime/config/state/effect readiness. Re-establish only deciding evidence; boundary-critical unknowns prevent ready, while genuinely method-only premises may remain for Plan's bounded conditional first work.
...
Write `docs/planning/work/<kebab-case-slug>/SCOPE.md` under the exact canonical Project Root. The direct artifact uses `Schema: iis-scope/v2`, `Project-Root` and `Status`. Use `draft` while product decisions remain unresolved and `ready` once meaning and executable observation are sufficient. `done` is Main's status-only completion record after current required evidence closure, independent attacks where applicable, effect settlement and authority checks under `iis-workflow`, not a planning or implementer write. `superseded` records an unconsumed replacement without rewriting delivered history. Closure BLOCKED is not a new Scope status.

Include:
- `## Product Authority`: one `iis-sources` JSON fence containing executor-owned `{snapshot, path}` references to exact Product Thesis originals. Snapshot IDs are allocated by the trusted store and are not derived from content. Reuse original revisions; do not create a per-Scope copy. Its binding meaning includes any adopted construction decision in that fixed source that constrains the current result.
- `## Transition Authority` only when an approved transition contract applies: bind its exact executor-owned original with the same `iis-sources` reference shape. Record applicability, selected Block and relevant conditions in Outcome/Acceptance; a stored reference does not grant approval or continuation authority.
...
Use `tools/validate_scope.py /absolute/path/to/SCOPE.md` for structure-only validation. `VALID` proves the v2 shape, not fixed-source currentness, Product Thesis closure, semantic completeness or user approval. Before a ready Scope enters Planner, Implementer or Assurance, the trusted host must run the common admission check, which resolves the stored Product Authority and requires a closed Product Thesis lifecycle. Check semantic fidelity directly: could this Scope pass while its adopted current product obligation is false? Resolve omissions without inventing unassigned requirements. Existing user-directed approval gates remain at their owning boundaries; faithful serialization adds no new approval.
...
The Assurance Baseline maps authored obligations to direct observations/gates and actual attack boundaries without making Probers whole-Scope approvers. Return concrete contract gaps to Main for the owning Thesis, Transition or Scope correction under `iis-workflow`. Do not add ad hoc Acceptance, weaken requirements to force closure or regard internal test success/no-finding as product completion.

## Contract revision and continuation

New evidence may change Main's construction choice only where adopted originals leave it open. If current Scope needs a Thesis-bound decision that is unresolved, it cannot be ready; if new evidence refutes a bound Thesis construction decision, return to Thesis before dependent Scope/Plan changes. Fixed transition geography changes first require its Transition revision; remaining open execution method belongs to Plan under `iis-workflow`. Preserve assigned Goal, required items and source history. Never change Outcome/Acceptance as a method edit or overwrite a delivered Scope; changed contracts require affected planning before dependent implementation. Historical success is not current integration evidence.

Keep the exact Scope path, fixed bound originals, chosen outcome, structure-validation result and material open limits available to the requested next stage. A ready Scope goes to execution planning only through common host admission when the current request permits it, without a separate Shaper approval role. No shared-understanding document, Spec, Ticket Set, ready-for-matt transition or duplicate Increment handoff is generated.
