...
Product-Thesis exploration or a supplied Completion Brief may identify a gap, but neither is a required handoff artifact or a readiness verdict. Consume exact adopted sources and deciding evidence at their actual strength. Keep evidence-only unknowns separate from unresolved product choices. A missing fact changing the promised boundary prevents ready; a method-only premise may remain for Plan's bounded conditional first work. After meaning changes, reassess affected Outcome/Acceptance through existing re-entry rules. Do not silently bind latest Thesis, refresh hashes to hide drift, import every explored sibling or rewrite completed history.

## Transition contract

When an approved BASELINE-NNN applies, reconcile the current measured Block and actual state against its entry/exit/readback, invariants, continuation/abort limits and remaining Goal obligations. A Block number does not establish runtime/config/state/effect readiness. Re-establish only deciding evidence; boundary-critical unknowns prevent ready, while method-only premises may remain for Plan's bounded conditional first work.
...
New evidence may change Main's construction choice only where adopted originals leave it open. Fixed transition geography changes first require its Baseline revision; product meaning belongs to Thesis and method to Plan under `iis-workflow`. Preserve assigned Goal, required items and source history. Never change Outcome/Acceptance as a method edit or overwrite a delivered Scope; changed contracts require affected planning before dependent implementation. Historical success is not current integration evidence.
