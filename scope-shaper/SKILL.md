...
Write `docs/planning/work/<kebab-case-slug>/SCOPE.md` under the exact canonical Project Root. The direct artifact uses `Schema: iis-scope/v1`, `Project-Root` and `Status`. Use `draft` while product decisions remain unresolved and `ready` once meaning and executable observation are sufficient. `done` is Main's status-only completion record after current required evidence closure, independent attacks where applicable, effect settlement and authority checks under `iis-workflow`, not a planning or implementer write. `superseded` records an unconsumed replacement without rewriting delivered history. Closure BLOCKED is not a new Scope status.

Include:
- `## Product Authority`: one line per exact project-local Thesis source, `- /absolute/path sha256:<full UTF-8 digest>`. Reuse original revisions; do not create a per-Scope copy. Its binding meaning includes any adopted construction decision in that exact source that constrains the current result.
