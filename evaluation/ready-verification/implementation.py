...
def run_one(item: dict, protocol: dict) -> dict:
    ...
    observations = [observe(argv, root) for argv in [metadata["readback_argv"], *metadata.get("additional_trigger_argv", [])]]
    if observer:
        observations.append(observe(observer, root))
    ...
    for file in sorted(runtime_data.rglob("*.json")) if runtime_data.exists() else []:
        ...
        if state.get("kind") == "execution" and state.get("project_root") == str(root):
            executions.append({"path": str(file), "sha256": hashlib.sha256(file.read_bytes()).hexdigest(),
                               ...
                               "mutation_revision": state.get("mutation_revision"),
                               "latest_evidence_revision": state.get("latest_evidence_revision")})
