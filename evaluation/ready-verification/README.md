...
`run_agent.py --stage` is mandatory. Only `prepare`, `implement`, `verify`, and `adaptive` load the Ready boundary extension. Product `plan` and read-only completion assessment do not create delivery execution/session state. Boundary-enabled stages receive the exact bundle `IIS_READY_VALIDATOR_PATH` and `IIS_READY_BUNDLE_ID`; there is no IIS runtime-data directory. Requested extension/bundle identity is recorded separately from actual loaded-host evidence; source identity is not claimed as a host load check.
