...
Historical v3 preparation used a generator checksum to detect the stale-module incident described above. Current IIS evaluation producers do not carry that mechanism forward: prospective fixtures are captured as executor-owned originals or compared directly with the source bytes used for preparation, and a changed generator requires separately prepared inputs. Existing historical metadata remains readable for reporting but is not upgraded into a current execution credential.
