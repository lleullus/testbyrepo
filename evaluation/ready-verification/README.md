...
Custom skill discovery receives parent directories, not individual skill directories. Home/project discovery is disabled. Native extension tools (`tools.xdev=false`) avoid the existing `xd://` write-wrapper admission conflict. `run_agent.py` explicitly supplies the isolated extension, workflow path and runtime data root; a separate profile alone is not sufficient isolation.
