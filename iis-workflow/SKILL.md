...
During the normal successful Verify → Coverage path, routine progress or a partial Coverage finding is not a terminal review and does not by itself authorize changing the stable product/config/tooling target. Let the current read-only Coverage invocation reach its bounded terminal result before corrective mutation so findings and limits remain attributable to one target. If urgent safety, data-loss or external-effect containment, an independent target change, or an actually blocked/unavailable review cannot wait for normal completion, preserve the in-flight review's partial evidence and limits, stop or let that invocation settle without treating it as `COMPLETE`, and restart corrective re-entry on the new target. This does not require Coverage to search remote possibilities or delay authorized containment.
