...
- Verification or Probe cannot observe a required boundary: return the exact evidence/mechanism gap, not an inferred product defect. Invocation-local verification usage belongs to verifier follow-up; shared project tooling to its authorized implementation/tooling actor; IIS-owned tools to their maintainer under separate maintenance authority. A Probe-only production-access limit follows `production-heuristic-probing` and does not authorize product mutation.
