...
The runners are the OMP process boundary. Start one managed runner for Implement, follow its stderr progress with the process manager's cursor-based event stream before exit, deduplicate any replayed log lines by the runner-owned monotonic progress sequence, inspect its single stdout terminal envelope and stable target, then start the Verify runner as a separate fresh process and follow it the same way. Auditor handoffs and detailed auditor lifecycle remain internal to that Pi owner; only aggregate auditor status/fan-in crosses the progress boundary. OMP cancellation terminates the managed runner, which forwards the signal to the complete Pi process group.
