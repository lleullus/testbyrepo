...
- `iis-adaptive-planning/`: explicit opt-in planning mode whose ownership still ends at Ready Tickets while the Outer Main defaults the current Increment through separate implementation, heuristic probing, verification, and authority-based corrective re-entry unless the user selects a stop override.
- `repo-snapshot/`: independent Git working-tree snapshot skill.
- `companion-skills/`: version-controlled companion skills that remain outside IIS Planning authority. Repository investigation/evidence grounding, Ready Ticket implementation, heuristic probing, verification, and shared review discipline live here as canonical sources while their live Codex entries are linked from `~/.codex/skills/`.
...
The five `companion-skills/` entries are not managed by these copy-sync scripts. Their canonical directories live in this repository and the corresponding `~/.codex/skills/<name>` entries are direct directory symlinks to those sources.

## Adaptive execution boundary

`IIS Adaptive Planning` remains one explicit opt-in skill; there is no separately invoked Adaptive Run skill. The **Outer Main** is the main agent for the current Adaptive invocation: it carries the invocation-local Run Contract and routes exact owner results without becoming a second planning, implementation, heuristic-exploration, or verification authority. IIS Planning still ends at the validated current Ready Ticket Set, while Outer Main continues that current Increment through separately installed Ready Ticket implementation, heuristic-probe, and verification skills by default unless the user explicitly selects planning-only/stop-at-Ready-Tickets/no implementation/no verification. Delivery execution defaults to SUBAGENT; use DIRECT only when the current user explicitly selects it for the relevant stage. Main remains Heuristic Probe Lead with dynamically admitted lane workers. Implementation and verification SUBAGENT execution is checkpointed: Outer Main returns bounded `CONTINUE | STEER | STOP` decisions at nonterminal phase boundaries without becoming a second implementer/verifier, and delivery routing or verification triage waits for the exact terminal owner result. With no narrower stop or broader outcome assignment, the default terminal is `CURRENT_INCREMENT_DELIVERED`. Corrective re-entry after a material correction is the Adaptive default; success continuation beyond the current Increment remains separately controlled by the Mandate's Continuation Authority.

The delivery skills keep their own authority and the verifier alone owns guarded `ready -> done`. Adaptive carries the relevant invocation-local model selection and bounded evidence-economy instruction through the current delivery owner contract; it adds no Graph database, scheduler, retry ledger, evidence budget, or extra Spec/Ticket review status.
...
Ready Tickets are the interface to delivery. Baseline IIS stops at that boundary. Under explicit IIS Adaptive Planning, Outer Main follows each exact Ready Ticket through separate implementation when enabled, the required heuristic-probe gate whenever Verification is enabled, and final verification under each owner's own contract; IIS still does not own delivery execution, heuristic findings, scheduling, or verdicts. `Verification: yes` is the existing Run Contract field for this probe-then-verify delivery path rather than a new heuristic field. When the verification lifecycle actually satisfies a Ticket's authored completion obligations it may mark the existing Ticket `Status: done`; IIS itself never infers delivery completion.
