...
- `iis-adaptive-planning/`: explicit opt-in planning mode whose ownership still ends at Ready Tickets while the Outer Main defaults the current Increment through separate implementation/verification and authority-based corrective re-entry unless the user selects a stop override.
...
Ready Tickets are the interface to delivery. Baseline IIS stops at that boundary. Under explicit IIS Adaptive Planning, the Outer Main hands each exact Ready Ticket to the separate implementation and verification skills under their own contracts by default unless the user selected a stop override; IIS still does not own delivery execution, scheduling, or verdicts. When the verification lifecycle actually satisfies a Ticket's authored completion obligations it may mark the existing Ticket `Status: done`; IIS itself never infers delivery completion.
