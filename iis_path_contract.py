...
def require_owned_directory(path: Path, *, writable: bool, trusted_uid: int | None = None) -> None:
    try:
        details = path.lstat()
    ...
    owner = os.geteuid() if trusted_uid is None else trusted_uid
    if details.st_uid != owner:
        raise PathContractError(f"directory is not owned by the trusted host user: {path}")
...
def require_canonical_owned_directory(path: Path, *, writable: bool, trusted_uid: int | None = None) -> Path:
    require_owned_directory(path, writable=writable, trusted_uid=trusted_uid)
    try:
...
def canonical_project_root(raw: str, *, writable: bool, trusted_uid: int | None = None) -> Path:
    path = Path(raw)
    if not path.is_absolute():
        raise PathContractError("project root must be absolute")
    return require_canonical_owned_directory(path, writable=writable, trusted_uid=trusted_uid)
