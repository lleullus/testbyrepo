...
독립 Plan Review가 이 exact Plan bytes를 `ADMIT`하고 권위/currentness가 유지된 뒤 구현자가 시작할 수 있는 범위는 다음뿐이다.

1. canonical composition schema/parser와 단일 PIL renderer;
2. current snapshot → source byte 검증 → materialization → 기존 registration transaction → authoritative readback을 연결하는 좁은 service;
3. 기존 `accept_composition` writer가 같은 schema를 정규화하도록 하는 clean cutover, 현재 test callsite 이관, 필요한 package export;
4. 이 Scope의 실제 local PIL/filesystem/SQLite self-check와 회귀.
...
### 2.1 EXISTING — 현재 writer/reader/state/effect
...
- percentage 값은 JSON number, finite, 소수점 이하 최대 4자리다. `x_pct/y_pct >= 0`, `w_pct/h_pct > 0`, `x_pct+w_pct <= 100`, `y_pct+h_pct <= 100`; `font_size_pct > 0`, `outline_width_pct >= 0`, `0 <= padding_pct < 50`, `line_spacing_pct >= 0`를 강제한다. RGBA는 exact `#RRGGBBAA`; align은 `left|center|right`다. normalize 후에도 모든 값은 explicit canonical keys로 남고 fallback style을 주입하지 않는다.
...
- `render_canonical(state, composition_revision, cuts, font_bytes) -> RenderedArtifactBytes` — `cuts` key/order가 정확히 `1..5`인지, 각 source bytes/hash/decode가 유효한지 다시 검사하고 §3 renderer로 PNG bytes/hash/closure를 반환한다. 네 컷 이하 또는 duplicate/extra cut은 render 전에 실패한다.
...
- `materialize(expected_authority_revision: int, expected_composition_revision: int) -> MaterializedArtifact`
  ...
  2. `cuts`가 정확히 `[1..5]`, 모두 `CURRENT`, `realization_complete.complete is True`, 각 realized identity/path/hash non-null인지 검사한다.
  3. composition을 normalize하고 font bytes/hash를 고정한다.
  4. source 5개의 actual bytes를 한 번만 읽고 DB `realized_content_hash`와 대조한 뒤 실제 PNG full decode/positive dimensions를 모두 성공시킨다. 하나라도 실패하면 output file/DB artifact는 0개다.
  5. render/save/validate/register/readback은 §5 순서를 따른다.
- `read_artifact(artifact_id: str) -> MaterializedArtifact` — fresh snapshot의 `review_artifacts` row를 먼저 찾고 derived final path를 읽는다. actual SHA-256이 row `content_hash`와 같고 Pillow full decode가 RGB PNG 1024×7680이며 embedded closure가 row composition/five-cut closure와 일치할 때만 bytes/path를 반환한다. directory file presence나 caller hash만으로 등록을 창조하지 않는다.
...
| staging PNG | CompositionService | post-save full decode/hash | promotion 또는 모든 handled failure에서 exact owned path unlink |
| immutable final artifact file | content-addressed no-overwrite promotion | `read_artifact`의 DB row + actual bytes/hash/decode/metadata | registered file 삭제/overwrite 없음 |
...
1. **Authoritative capture** — one fresh snapshot에서 global revision `R`, composition revision `C`, normalized state와 ordered cut closure를 잡는다. snapshot 밖에서 caller-supplied asset list를 섞지 않는다.
2. **Preflight all inputs** — font와 다섯 source의 bytes/hash/full decode를 모두 완료한다. 실패 시 staging/final/DB effect가 없다.
3. **Render staging** — project 내부 `.composition-staging/<random-id>/candidate.png`에 exclusive create하고 deterministic bytes를 쓴다. same filesystem에 두어 final promotion을 atomic하게 한다.
4. **Validate candidate** — staging을 별도 open해 non-empty, exact PNG/RGB/1024×7680, full load, embedded closure를 확인하고 file bytes SHA-256을 재계산한다. in-memory hash와 다르면 실패한다.
5. **No-overwrite promote** — hash로 artifact id/path를 정하고 same-filesystem hard-link/exclusive destination create로 final path를 만든다. 이미 path가 있으면 overwrite하지 않고 existing full bytes/hash/metadata가 exact same인지 확인한다. staging link/file은 즉시 제거한다.
...
7. **Authoritative readback** — 새 connection snapshot에서 exact artifact id/hash/composition/five closure를 찾고 `read_artifact`로 final bytes를 다시 hash/decode한다. current composition/realization closure도 여전히 일치할 때만 successful current receipt를 반환한다.

파일을 먼저 만들고 DB를 나중에 등록하는 이유는 등록 row가 missing bytes를 authoritative artifact로 가리키는 더 위험한 상태를 만들지 않기 위해서다. process가 promotion 뒤 DB commit 전에 죽으면 unregistered content-addressed file이 남을 수 있으나 어떤 product reader도 이를 artifact로 보지 않는다. 다음 동일 materialization은 exact bytes를 검증해 재사용할 수 있다. 이 Scope에서 broad directory GC/repair framework를 추가하지 않는다.

### 5.2 Handled failure와 partial-effect cleanup

- preflight/render/save/decode 실패: exact staging subtree만 제거; final/DB 0 effect.
- destination create 전 실패: staging만 제거.
- final을 이 invocation이 새로 만들고 registration이 실패: fresh snapshot에서 exact id가 등록되었는지 먼저 확인한다. 등록되지 않았고 path bytes가 자신이 만든 hash와 같을 때만 final을 unlink한다. 다른 invocation이 exact artifact를 등록했다면 shared final을 삭제하지 않는다.
- registration transaction 오류: SQLite rollback은 기존 API가 맡는다. compositor는 error를 success receipt로 변환하거나 DB row를 raw SQL로 보정하지 않는다.
- registration 성공 후 readback에서 bytes missing/hash mismatch/decode failure: registered identity를 임의 삭제하지 않고 `ArtifactReadbackError`로 실패하며 corruption evidence(path, expected/actual hash)를 보존해 Safe Abort한다. 과거 realization/intent/composition은 건드리지 않는다.
- cleanup 실패는 원래 오류와 함께 보고하고 success를 반환하지 않는다. owned path만 대상으로 하고 realization/historical registered artifact를 scan-delete하지 않는다.

### 5.3 Concurrent materialize/composition/realization changes

- **composition 또는 realization이 render 중 변경**: global `R` CAS 또는 registration의 `C`/closure check가 commit을 거절한다. newly owned unregistered final을 §5.2로 제거하고 caller에게 conflict를 반환한다. old snapshot으로 자동 rerender/retry하지 않는다.
- **동일 closure의 동시 materialize**: deterministic bytes/id/path가 같다. one no-overwrite creation winner만 path owner다. DB registration winner가 commit한다. loser는 conflict/unique 오류 후 fresh readback에서 current `C`, current closure, exact registered id/hash와 bytes가 모두 같은 경우에만 idempotent same artifact receipt를 반환한다; 다르면 conflict다.
- **registration 직후 composition change**: registration은 그 commit 순간의 valid historical artifact이고 file/row를 지우지 않는다. final readback에서 current `C`/closure가 달라졌으면 `ArtifactNoLongerCurrentError(artifact_id)`를 반환해 current review 성공으로 위장하지 않는다. 새 composition은 새 materialization을 요구한다. 이후 `authorize_release`도 기존 current-closure check로 old artifact를 거절한다.
...
1. `composition.py`에 exact schema normalization, Decimal edge math와 pure slot/geometry helpers를 먼저 구현한다. arbitrary-N/list-order override나 fallback style은 넣지 않는다.
2. five cut decode/hash/contain stitch와 bubble shape/text engine을 연결한다. 한 cut/font/text overflow 실패가 render 전체를 중단하는지 in-memory/output-absence로 먼저 확인한다.
3. deterministic PNG metadata/save/readback과 content-addressed id/path를 구현한다.
4. `composition_service.py`에 §5 orchestration을 연결하고 기존 `register_review_artifact`를 그대로 호출한다. raw SQL artifact writer는 만들지 않는다.
5. `store.accept_composition`을 normalizer에 clean-cutover한다. `str` 입력과 arbitrary dict acceptance를 제거하고 현 test composition callsite를 canonical fixture로 모두 이관한다. compatibility alias/deprecated path는 두지 않는다.
6. `__init__.py` export를 추가한다. CLI/schema/migrations/generation은 실제 필요가 없으므로 그대로 둔다.
7. load-bearing `tests/test_composition.py` regression과 기존 transaction regression을 정리한다. source-text/wiring assertions나 mock pixel success는 두지 않는다.
8. §7 disposable real-boundary scenarios를 실행하고 exact effect paths/bytes/hashes/SQLite before-after를 verifier handoff에 남긴다. 이 self-check는 semantic verification/Exit verdict가 아니다.
...
2. **Exact five / B / Exit 2** — current cut `1..5` actual PNG로 materialize 성공시킨다. direct renderer에 4, duplicate five, extra sixth를 각각 주면 staging/final/DB artifact 없이 실패해야 한다. service는 caller cut list를 받지 않고 snapshot order만 사용함을 actual receipt closure로 확인한다.
...
5. **Canonical pixels and identity / E–G / Exit 6–8** — distinct-color/marker five PNG, nonzero gap, ellipse+rounded rectangle, Korean/CJK/Latin multiline text를 materialize한다. decoded artifact에서 cut order, exact slot/gap pixels, contain margins, bubble bounds와 nonblank text glyph pixels을 읽는다. DB row의 artifact id/hash/composition revision/five closure, embedded desired/realized/asset/source hashes와 actual file SHA-256을 모두 대조한다. 새 process의 `read_artifact`도 같은 bytes를 반환해야 한다.
6. **Determinism and font/text failures / I / Exit 10–11** — same source/state/font/runtime을 두 번 render해 exact PNG bytes/hash equality를 확인한다. explicit newline, whitespace wrap, one overlong token hard-wrap를 decoded output과 recorded expected line positions로 확인한다. missing/unreadable font, wrong font hash, zero/non-finite/too-small geometry, content box overflow를 각각 실행해 auto-fallback/shrink/truncate 없이 전체 실패하고 artifact row/file가 없는지 본다.
...
8. **Handled rollback / Atomic 13.4** — staging save, post-save decode, final creation, registration 전/중 SQLite exception을 한 번씩 실제로 유발한다. transaction row 0, owned staging/final cleanup, source/old artifacts 불변을 확인한다. registration 후 final bytes를 고의 손상시키는 readback scenario는 success가 아니라 `ArtifactReadbackError`이며 DB를 임의 삭제하지 않는지 확인한다.
9. **Concurrent composition/realization race** — renderer를 실제 barrier에서 멈춘 뒤 별도 writer가 `accept_composition` 또는 current cut의 new realization을 commit한다. old call은 registration conflict, no registered old closure, owned unregistered final cleanup이어야 한다. 별도 scenario에서 registration commit 직후 composition writer를 이겨 historical artifact가 남되 final current receipt가 거절되고 bytes는 immutable인지 확인한다.
10. **Duplicate materialize race** — 두 independent service/process가 같은 `R/C/closure`에서 실제 render를 경쟁한다. exactly one registration row와 one content-addressed file만 남고 둘 다 성공한다면 exact same id/hash/bytes를 반환해야 한다. 다른 file overwrite나 duplicate row, cleanup winner의 registered file deletion이 없어야 한다.
11. **Downstream identity boundary / J–K / Exit 12–13, PATH-7/11** — materialize receipt의 `artifact_id`만 사용해 별도 review-like reader와 export-like reader가 `read_artifact`를 호출한다. 두 read가 DB의 같은 row/path/full bytes/hash/decoded geometry를 얻고, API에 text/layout input 또는 renderer 호출이 없음을 확인한다. 실제 UI/export/Blogger 실행이나 전달 성공은 주장하지 않는다.
12. **Restart and filesystem false-success** — 모든 process/connection을 닫은 뒤 새 Python process가 snapshot+`read_artifact`로 same identity/hash/bytes를 읽는다. unregistered content-addressed file을 따로 두어도 reader가 거절하는지, DB row만 있고 missing/corrupt file이면 readback failure인지 확인한다.
...
Implementation owner hands verifier: exact current Thesis/Scope/Baseline/Plan digests; target source/test hashes; Python/Pillow/SQLite/font path+font hash+feature versions; each disposable scenario command/input; before/after fresh DB snapshots and relevant raw rows; source/font/artifact actual byte hashes; decoded dimensions/pixel boxes/text nonblank observations; staging/final created-path ownership; concurrent writer outcomes; and cleanup result. Internal return, HTTP acceptance, screenshot, mock, metadata-only registration 또는 file presence alone은 proof가 아니다.

Verifier target은 fresh implementation bytes와 fresh disposable project다. scenario 중 font/Pillow environment가 unavailable하면 geometry/stitching의 관찰과 typography `INCONCLUSIVE`를 분리하고 surrogate success로 전체를 통과시키지 않는다. known partial/orphan/corruption 또는 unresolved invariant가 있으면 BLOCK-04 handoff는 금지된다.
...
- `dependent_work_not_yet_permitted`: raw SQL/새 artifact transaction writer, schema migration, changed filesystem/DB atomic strategy.
...
Private helper/dataclass 이름, error message wording, local cache container, test fixture 색상/문구, equivalent Pillow drawing call의 작은 배치는 재량이다. surface dimensions, contain/no-crop, integer slot formula, half-up edge math, explicit schema, exact font hash/no fallback, fail-on-overflow, content-addressed no-overwrite path, existing registration transaction, cleanup ownership 및 authoritative readback은 재량이 아니다.
...
- filesystem과 SQLite는 하나의 atomic transaction이 아니다. 이 Plan은 missing authoritative bytes보다 non-authoritative orphan 가능성을 택하고, handled failure는 ownership-aware cleanup하며, DB-first/readback 없이 file presence를 성공으로 보지 않는다.
...
- `register_review_artifact` 대체/우회, schema migration, artifact path/identity/hash 의미나 filesystem–DB ordering 변경;
- multiple renderer/downstream reflow, mutable overwrite path, partial four-cut output 또는 stale current 허용;
...
이 문서는 exact ready BLOCK-03의 실행/자체확인 방법만 준비한다. product source/data/schema, Thesis, Scope, Baseline, completed Scope/status, UI/release/external system을 변경하지 않았고 implementation, semantic verification, Plan Review decision 또는 BLOCK-03 Exit를 발행하지 않는다. 다음 단계는 Main이 별도 Gemini Flash invocation으로 이 exact Plan bytes를 독립 검토하는 것이다. `ADMIT`과 currentness 확인 전에는 §1.2 시작 범위를 실행하지 않는다.
