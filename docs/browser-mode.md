...
- `--browser-thinking-time <light|standard|extended|heavy|pro>`: set ChatGPT effort (modern UI: Instant, Medium, High, Extra High, Pro). You can also set a default in `~/.oracle/config.json` via `browser.thinkingTime`.
- Explicit Pro effort and GPT-5.5 Pro Extended are verified from ChatGPT's effort controls. A run **fails closed** if either cannot be confirmed, and any detected model mutation also aborts before submission. Detection failures write a bounded, redacted model-picker diagnostic to the normal session log.
