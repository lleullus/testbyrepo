# Phase 3 Oracle follow-up 1 prompt

같은 Phase 3 검토를 이어간다. DevSpace MCP plugin으로 아래 최신 파일을 다시 직접 읽고 파일을
수정하지 마라.
...
1. F1 correction이 같은/overlapping canonical Project-Root의 source-changing IMPLEMENT만 hidden
   occupancy로 직렬화하면서 실제 overwrite/attribution failure를 닫았는가.
2. occupancy를 active transition 시작이 아니라 first Worker dispatch 직전에 acquire하는 더 작은
   correction이 안전한가. source currentness 재확인과 occupancy atomic decision의 의미가 충분한가.
3. no-op 구현, 다른 physical domain, VERIFY/inspect를 직렬화하지 않은 경계가 올바른가.
4. F2를 `REJECT_AS_SCOPE_EXPANSION`하고 exact canonical Ticket locator를 work identity authority로
   유지하며 rename/move/copy를 새 work로 정한 것이 Phase 1/2 historical recovery와 모순되는가.
5. mutation-domain occupancy가 public claim/lease나 global/path lock graph를 이름만 바꿔 재도입했는가.
6. 전체 Phase 3 계약에 다른 material concurrency/restart/publication 결함이 남았는가.

F1에 `CLOSED|STILL_OPEN|REGRESSED`, F2 거부에 `REJECTION_JUSTIFIED|REJECTION_UNSAFE`를 판정하라.
새 finding에는 필수 여섯 항목과 최소 correction/단계 위임을 포함하라. 새 finding과 unresolved user
decision이 없으면 Phase 3가 `DESIGN_CONVERGED` 가능한지 명시하라.
