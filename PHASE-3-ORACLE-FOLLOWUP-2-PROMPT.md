# Phase 3 Oracle follow-up 2 prompt

같은 Phase 3 검토를 계속한다. DevSpace MCP plugin으로 아래 최신 파일을 다시 직접 읽고 파일을
수정하지 마라.
...
1. mutation-domain occupancy를 먼저 획득하고 유지한 채 source를 다시 읽어 expected source와
   일치할 때만 Worker dispatch한다는 ordering이 F1 TOCTOU를 닫았는가.
2. Candidate publication 시 result append, predecessor/head, occupancy release, active transition
   close가 같은 atomic commit에 포함된다는 의미가 pre/post release crash를 닫았는가.
3. mismatch 처리, capture algorithm과 failpoints를 Phase 4/7에 위임해도 Phase 3 의미가 충분한가.
4. F2 거부 및 rename/move/copy 새-work 경계에 회귀가 없는가.
5. 전체 Phase 3 계약에 새 material finding, 불필요한 legacy ceremony 또는 unresolved user decision이
   남았는가.

F1에 `CLOSED|STILL_OPEN|REGRESSED`, F2에 `REJECTION_JUSTIFIED|REJECTION_UNSAFE`를 판정하라.
새 finding에는 필수 여섯 항목과 최소 correction/단계 위임을 포함하라. 새 finding과 사용자 결정이
없으면 Phase 3가 `DESIGN_CONVERGED` 가능한지 명시하라.
